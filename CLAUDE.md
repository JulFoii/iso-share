...
- **`lib/metadata.js`**: per-file sidecar JSON at `uploads/.meta/<name>.iso.json` (checksum, ISO info, download counter, free-text tags). `listFiles()` filters on `.iso`, so the sidecar directory is invisible in listings. Download counts are buffered in memory and flushed periodically / on shutdown, not written synchronously per request. Tags are a plain `tags: string[]` field on the same sidecar, written through the generic merge-`update()` — no schema change needed there; tags are assigned only by `lib/auto-tags.js` (see below), there is no manual add anymore, so `safeTag()` in `lib/safe-name.js` only guards the candidate strings auto-tags produces and the `:tag` path segment on `DELETE /files/:name/tags/:tag`. `listFiles(query)` matches the search term against both name and tags, so `/search`/`/admin-search` and a tag's own badge link double as a tag filter; `server.js`'s `listAllTags()` collects every distinct tag across all files (independent of the current search) for the tag-filter bar in `views/partials/tag-filter.ejs`. `autoTags`/`removedAutoTags` are the bookkeeping fields for `lib/auto-tags.js` — not shown in the UI, just used to reconcile `tags` on the next hash run
...
- **`lib/hash-queue.js`**: background, one-at-a-time SHA-256 + ISO-info worker. `scanAll()` runs at startup and picks up files dropped into `uploads/` outside the app (e.g. `rsync`); a checksum is considered stale (and gets recomputed) whenever the file's size/mtime no longer match what was hashed. The same run also (re-)applies auto-tags via `lib/auto-tags.js`, since the ISO info it needs is already being read here. For a file whose checksum is already current but whose sidecar predates the auto-tags feature (no `autoTags` field yet), `scanAll()` backfills tags straight from the already-stored `iso` info instead of enqueueing a full rehash — so upgrading to auto-tags doesn't require re-reading every existing upload
